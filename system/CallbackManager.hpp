...
template <class CBGroup, CBGroup CBType, typename ... Parameters> requires
    std::is_enum_v<CBGroup>
...
template <>                             \
struct IsCallbackHandle<group, cbtype, __VA_ARGS__>   \
{                                       \
        ...
        registeredHandles(other.registeredHandles.size())
{
    ...
    for (; that_it != other.registeredHandles.cend(); ++this_it, ++that_it)
    {
        ...
        auto* new_handle = (VirtualCallbackHandle<CBGroup>*)std::malloc(ptr->size);
        if (!new_handle)
        {
...
ExternalHandle subscribeCallback(std::function<CallbackReturn(Params...)> func)
{
    ...
    std::unique_ptr<VirtualHandle> handle((VirtualHandle*)(
        new HandleType{ {Type}, func })
    );
...
ExternalHandle subscribeCallback(std::function<CallbackReturn(ThisT*, Params...)> func, ThisT *this_ptr)
{
    static_assert(IsCallbackHandle<CBGroup, Type, Params...>::value,
        "Function does not have a valid signature for this callback"
    );
    ...
    std::unique_ptr<VirtualHandle> handle((VirtualHandle*)(
        new HandleType{ {Type}, func, this_ptr })
    );
...
void invokeCallback(Params... params)
{
    ...
    for (auto&& handle : registeredHandles)
    {
        if (handle->type == Type && !handle->markedForDeletion)
        {
            ...
            if (handle->isMember)
            {
                ...
                auto bound = std::bind( real_handle->function, real_handle->this_pointer, params... );
                ret = bound();
            }
            else
            {
    ...
    registeredHandles.erase(std::remove_if(
            registeredHandles.begin(), registeredHandles.end(),
            [](auto&& handle) { return handle->markedForDeletion; }
...
template <CBGroup Type, typename ... Params>
static ExternalHandle SubscribeCallback(std::function<CallbackReturn(Params...)> func)
{
    static_assert(IsCallbackHandle<CBGroup, Type, Params...>::value, "Function does not have a valid signature for this callback");

    using HandleType = CallbackHandle<CBGroup, Params...>;

    std::unique_ptr<VirtualHandle> handle((VirtualHandle*)(new HandleType{ {Type}, func }));
...
ExternalHandle SubscribeCallback(std::function<CallbackReturn(ThisT*, Params...)> func, ThisT *this_ptr)
{
    ...
    std::unique_ptr<VirtualHandle> handle((VirtualHandle*)(new HandleType{ {Type}, func, this_ptr }));
...
template <class CBGroup>
std::vector<std::unique_ptr<typename StaticCallbacks<CBGroup>::VirtualHandle>> StaticCallbacks<CBGroup>::RegisteredHandles;
