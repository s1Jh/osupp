           ...
           bool alphablend)		//use alpha blend or not
{
    ...
    float line_vertex[]=
            {
                    float(x1-tx-Rx-cx), y1-ty-Ry-cy, //fading edge1
                    float(x2-tx-Rx+cx), y2-ty-Ry+cy,
                      float(x1-tx-cx,y1-ty-cy),	  //core
                            float(x2-tx+cx,y2-ty+cy),
                                  float(x1+tx-cx,y1+ty-cy),
                                        float(x2+tx+cx,y2+ty+cy),
                                              float(x1+tx+Rx-cx, y1+ty+Ry-cy), //fading edge2
                                                    float(x2+tx+Rx+cx, y2+ty+Ry+cy)
            };
