...
std::string convertFailureEnum(Output reason)
{
    ...
    {
        case CouldNotOpen:
            return "Could not open the file.\nNote that special characters in the file path may also result in this error. If that is the case, make sure the file and the directory it is in doesn't have any non ASCII characters in it's name.";
        case CouldNotSave:
            return "Could not create the output file.\nCheck if you have write permissions to the directory where the map importer located.";
        case NotOsuFile:
            return "File does not appear to be a OSU! map file\nThis file does not appear to start with the osu map file header.";
        case Unknown:
            return "Unknown reason.\nPlease, contact the developer with the version and the map file you were trying to convert.";
        case Success:
            return "wat";
        default:
            return "Failed to decode a failure reason.\nA error in the program caused a invalid error, please restart the program and try again.";
    }
}*/
...
bool LoadNote(std::vector<std::string> params, MapInfo& map)
{
    fvec2d pos = PosConversion(
            fvec2d{ std::stof(params[0]), std::stof(params[1]) }
    );
